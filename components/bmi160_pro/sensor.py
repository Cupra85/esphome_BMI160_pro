...
# ----- Metadaten -----
MULTI_CONF = True         # mehrere Instanzen erlaubt
DEPENDENCIES = ["i2c"]    # benötigt I2C
    ...
    cv.Schema(
        {
            ...
            # Schwellenwerte für C++-Seite
            cv.Optional(CONF_TILT_THRESHOLD, default=15.0): cv.float_range(min=0.0, max=90.0),
            cv.Optional(CONF_MOTION_THRESHOLD, default=0.3): cv.float_range(min=0.0, max=10.0),
            cv.Optional(CONF_VIBRATION_THRESHOLD, default=0.5): cv.float_range(min=0.0, max=50.0),
        }
    )
    .extend(i2c.i2c_device_schema(0x68))       # I2C Adresse
    .extend(cv.polling_component_schema("5s")) # update_interval
)
...
async def to_code(config):
    ...
    # registrieren als Komponente & I2C-Device
    ...
    # Sensor-Objekte übergeben
    if CONF_ACCEL_X in config:
        cg.add(var.set_accel_x_sensor(await sensor.new_sensor(config[CONF_ACCEL_X])))
    if CONF_ACCEL_Y in config:
        cg.add(var.set_accel_y_sensor(await sensor.new_sensor(config[CONF_ACCEL_Y])))
    if CONF_ACCEL_Z in config:
        cg.add(var.set_accel_z_sensor(await sensor.new_sensor(config[CONF_ACCEL_Z])))

    if CONF_GYRO_X in config:
        cg.add(var.set_gyro_x_sensor(await sensor.new_sensor(config[CONF_GYRO_X])))
    if CONF_GYRO_Y in config:
        cg.add(var.set_gyro_y_sensor(await sensor.new_sensor(config[CONF_GYRO_Y])))
    if CONF_GYRO_Z in config:
        cg.add(var.set_gyro_z_sensor(await sensor.new_sensor(config[CONF_GYRO_Z])))

    if CONF_PITCH in config:
        cg.add(var.set_pitch_sensor(await sensor.new_sensor(config[CONF_PITCH])))
    if CONF_ROLL in config:
        cg.add(var.set_roll_sensor(await sensor.new_sensor(config[CONF_ROLL])))
    if CONF_INCLINATION in config:
        cg.add(var.set_inclination_sensor(await sensor.new_sensor(config[CONF_INCLINATION])))

    if CONF_TEMPERATURE in config:
        cg.add(var.set_temperature_sensor(await sensor.new_sensor(config[CONF_TEMPERATURE])))
    if CONF_VIBRATION in config:
        cg.add(var.set_vibration_sensor(await sensor.new_sensor(config[CONF_VIBRATION])))
